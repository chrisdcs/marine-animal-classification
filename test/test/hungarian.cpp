...
Hungarian::Hungarian()
{
	...
	m_costmatrix.resize(m_rows, vector<double>(m_cols,0));
	m_assignment.resize(m_rows, vector<double>(m_cols,0));
}

Hungarian::Hungarian(const vector<vector<double> >& input_matrix, int rows, int cols, MODE mode)
{
  ...
  m_costmatrix.resize(rows, vector<double>(cols,0));
  m_assignment.resize(rows, vector<double>(cols,0));
...
void hungarian_print_costmatrix(const vector<vector<double> >& C, int rows, int cols)
{
	int i,j;
	fprintf(stderr , "\n");
	for(i=0; i<rows; i++) 
...
void hungarian_print_assignmentmatrix(const vector<vector<double> >& C, int rows, int cols)
{
	int i, j;
	fprintf(stderr, "\n");
	for (i = 0; i<rows; i++)
...
void Hungarian::print_assignment() {
  hungarian_print_assignmentmatrix(m_assignment, m_rows, m_cols) ;
}

void Hungarian::print_cost() {
  hungarian_print_costmatrix(m_costmatrix, m_rows, m_cols) ;
}
...
int Hungarian::init(const vector<vector<double> >& input_matrix, int rows, int cols, MODE mode)
{
...
bool Hungarian::check_solution(const vector<double>& row_dec, const vector<double>& col_inc, const vector<double>& col_vertex)
{
	int k, l, m, n;
...
bool Hungarian::assign_solution(const vector<double>& row_dec,const vector<double>&  col_inc, const vector<double>&  col_vertex)
{
...
bool Hungarian::solve()
{
	...
	vector<double> col_vertex(m), row_vertex(n), unchosen_row(m), parent_row(n),
	...
	m_assignment.assign(m, vector<double>(n, HUNGARIAN_NOT_ASSIGNED));
...
const vector<vector<double> >& Hungarian::assignment() const
{
	return m_assignment;
}
